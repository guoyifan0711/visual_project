...
################################################syz server part####################################################
  ...
  freq_table_terrorism = read_csv("gu.csv")
  
  
  output$cloud <- renderWordcloud2({
    if (input$source == "book") {
      ww = freq_table_terrorism
    } else{
      ww = create_freq_table(data_source())
    }
    
    ww <- ww %>% 
      filter(! word %in% c(input$words_to_remove1, input$words_to_remove2,
                                          input$words_to_remove3,input$words_to_remove4,
                                          input$words_to_remove5, input$words_to_remove6, 
                                          input$words_to_remove7, input$words_to_remove8, 
                                          input$words_to_remove9, input$words_to_remove10)) %>%
      arrange(desc(freq)) %>%
      head(input$num)
    
    wordcloud2(ww, backgroundColor = input$col)
